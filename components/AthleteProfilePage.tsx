...
import {
    ...
    HealthForm,
    SupplementsForm
} from './ProgressiveProfilingModal';
...
export const AthleteProfilePage: React.FC = () => {
    ...
    useEffect(() => {
        if (profile) {
            ...
            if (profile.measures) {
                setEditAltura(profile.measures.altura?.toString() || '');
                setEditPunho(profile.measures.punho?.toString() || '');
                setEditTornozelo(profile.measures.tornozelo?.toString() || '');
            }
        }
    }, [profile]);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    >
                        {isEditingStructural ? (
                            ...
                            <>
                                <InfoRow label="Altura" value={`${profile.measures?.altura || '-'} cm`} />
                                <InfoRow label="Punho" value={`${profile.measures?.punho || '-'} cm`} />
                                <InfoRow label="Tornozelo" value={`${profile.measures?.tornozelo || '-'} cm`} />
                            </>
                        )}
                    </EditableInfoCard>
